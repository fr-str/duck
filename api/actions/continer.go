...
func (a *Containers) SSRK(r *ws.Request) ws.Response {
	...
	case "start":
		ws.GoError(r, wsc.Codes{wsc.InternalServerError}, cont.Start)
	case "stop":
		ws.GoError(r, wsc.Codes{wsc.InternalServerError}, cont.Stop)
	case "restart":
		ws.GoError(r, wsc.Codes{wsc.InternalServerError}, cont.Restart)
	case "kill":
		ws.GoError(r, wsc.Codes{wsc.InternalServerError}, cont.Kill)
	}
...
func (a *Containers) ApplyDockerCompose(r *ws.Request) ws.Response {
	...
	ws.GoError(r, wsc.Codes{wsc.InternalServerError}, func() error {
...
func (a *Containers) Delete(r *ws.Request) ws.Response {
	...
	if !a.Force && !a.SoftForce && cont.State != "exited" {
		return ws.Error(r, nil, wsc.Forbbiden, wsc.ContainerIsRunning)
	}

	if a.SoftForce {
		ws.GoError(r, wsc.Codes{wsc.InternalServerError}, cont.Stop, cont.Delete)
	} else {
		ws.GoError(r, wsc.Codes{wsc.InternalServerError}, cont.Kill, cont.Delete)
	}
